<template>
  <div class="container-fluid" id="recordlistproduct-container">
    ...
    <Row type="flex" justify="center">
      <Table width="872" height="400" max-height="450" border ref="selection" :columns="columns" :data="datarecord" size="small" :loading="loading" v-if="this.switch111">
  ...
  methods: {
    ...
    confirm() {
      if (this.checkstatus === "นำเข้า") {
        del("/api-inv/import-p/" + this.deletingID).then((res) => {
          this.$Loading.finish();
      ...
      if (this.checkstatus === "นำออก") {
        del("/api-inv/export-p/" + this.deletingID).then((res) => {
          this.$Loading.finish();
